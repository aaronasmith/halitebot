...
namespace Halite
{
    class MySecondBot
    {
        public const string MyBotName = "A-ronbot2";
        ...
        public static Move Move(Location location)
        {
            ...
            // If we don't find any, pick a random direction that we own, or just sit still, preferrably still.
            var ownedAvailable = siteDictionary.Where(s => s.Value.Owner == myID).Select(s => s.Key).Concat(new[] { Direction.Still, Direction.Still }).ToList();
            return new Move(location, ownedAvailable[random.Next(ownedAvailable.Count)]);
        }
